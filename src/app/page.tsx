...
function HomePage() {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        // Call the Flask backend's /upload endpoint
        const response = await fetch('http://localhost:5000/upload', {
          method: 'POST',
          body: formData,
        });
        ...
        if (!response.ok) {
          throw new Error('Error uploading and processing video');
        }

        // Store the analysis in localStorage for use in the analysis page
        const { analysis } = await response.json();
        localStorage.setItem('geminiAnalysis', analysis);
      } catch (error) {
        console.error('Error processing video:', error);
      }
    }
...
function AnalysisPage() {
  ...
  return (
    <div className="analysis-page-container">
      ...
      <div className="chatgpt-analysis">
      {localStorage.getItem('geminiAnalysis') || 'No analysis available'}
      </div>
      <button className="back-button" onClick={() => navigate('/responsePage')}>Back to Workout Analysis List</button>
    </div>
  );
